...
public class DatabaseRelyingPartyRegistrationRepository implements
  ...
  public void loadRegistrations() {
    ...
    List<SamlConfig> enabledConfigs = samlConfigRepository.findByEnabledTrueWithOrganization();
  ...
  private RelyingPartyRegistration buildRelyingPartyRegistration(SamlConfig config) {
    ...
    // --- ADD THIS STRICT CHECK HERE ---
    if (config.isSignRequests() && signingCredential == null) {
      log.error("CRITICAL SAML ERROR: signRequests is TRUE, but signingCredential loaded as NULL.");
      log.error("Path: '{}', Alias: '{}', KeystorePassExists: {}, KeyPassExists: {}",
          config.getSpSigningKeystorePath(),
          config.getSpSigningKeyAlias(),
          StringUtils.hasText(config.getSpSigningKeystorePasswordEncrypted()),
          StringUtils.hasText(config.getSpSigningKeyPasswordEncrypted()));
      throw new ConfigurationException("Cannot build SAML Registration: signRequests is true but SP Signing Credential failed to load. Check paths and passwords in DB.");
    }
    // ----------------------------------
