...
public class AuthController {
  ...
  // This tells Spring where to save the context (Redis/HttpSession)
  private final SecurityContextRepository securityContextRepository =
      new HttpSessionSecurityContextRepository();

  @Operation(
      summary = "Initialize CSRF Token",
      description = "Fetches a fresh XSRF-TOKEN cookie required for subsequent POST/PUT/DELETE requests."
  )
  @ApiResponse(responseCode = "200", description = "CSRF cookie successfully attached to response")
  @GetMapping("/csrf")
  public Map<String, String> getCsrfToken(HttpServletRequest request) {
    // 1. Get the deferred token from the request
    CsrfToken csrfToken = (CsrfToken) request.getAttribute(CsrfToken.class.getName());

    if (csrfToken != null) {
      // 2. CRITICAL: Calling getToken() forces Spring to generate the token
      // and invoke the CookieCsrfTokenRepository to set the header!
      csrfToken.getToken();

      // 3. Return it in the body so you can visually confirm it
      return Map.of(
    ...
    return Map.of("error", "CSRF is disabled or bypassed for this endpoint");
  }
  /**
   ...
   */
  @Operation(summary = "Authenticate User (Session + JWT)",
      description = "Authenticates a user, sets a secure Redis session cookie, rotates CSRF, and returns user details/JWT.")
  @ApiResponses(value = {
      ...
      @ApiResponse(responseCode = "401", description = ApiErrorMessages.AUTHENTICATION_FAILED,
          content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiError.class))),
      @ApiResponse(responseCode = "403", description = ApiErrorMessages.ACCOUNT_LOCKED + " / " + ApiErrorMessages.ACCOUNT_DISABLED,
          content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiError.class))),
  ...
  public ResponseEntity<ApiSuccessResponse<LoginResponse>> login(
    ...
    log.info("Received login request for user/email: {}", loginRequest.getUsernameOrEmail());
    ...
    // 3. Explicitly save the context to create the Session (IAMSESSID)
    securityContextRepository.saveContext(context, request, response);

    // 4. Trigger Session strategy to rotate the CSRF token
    sessionAuthenticationStrategy.onAuthentication(authentication, request, response);

    // 5. CRITICAL FIX: Wake up the newly rotated deferred token to force the Set-Cookie header!
    CsrfToken csrfToken = (CsrfToken) request.getAttribute(CsrfToken.class.getName());
    if (csrfToken != null) {
      csrfToken.getToken(); // This line physically writes the new XSRF-TOKEN cookie
    }

    // 6. Execute your custom AuthService logic (JWT generation, checking password reset flags,
    // etc.)
    LoginResponse loginResponse = authService.login(loginRequest);

    // 7. Format the response message
    String successMessage = loginResponse.isRequiresPasswordChange() ?
        ApiResponseMessages.TEMPORARY_PASSWORD_LOGIN : ApiResponseMessages.LOGIN_SUCCESSFUL;

    ApiSuccessResponse<LoginResponse> successResponse = ApiSuccessResponse.ok(loginResponse, successMessage);
